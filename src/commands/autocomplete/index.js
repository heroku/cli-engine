  ...
  async run () {
    ...
    switch (shell) {
      case 'bash':
        ...
        this.out.log()
        this.out.log(CustomColors.cmd(`$ echo 'source ${path.join(this.autocompletePath, 'bash', 'heroku.bash')}' >> ~/.bashrc`))
        break
      case 'zsh':
        ...
        this.out.log()
        this.out.log('Run the following zsh command to ensure no permissions conflicts:')
        this.out.log()
        this.out.log(CustomColors.cmd(`$ compaudit`))
        break
      default:
