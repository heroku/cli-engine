...
test('links example-plugin', async () => {
  ...
  let existingFiles = []
  try {
    await run(['plugins:link', './example-plugin', '--force'])
    ...
    // add new command
    const newCommandPath = path.join('example-plugin', 'src', 'commands', 'foo', 'bar.ts')
...
export default class extends Command {
  ...
  async run() { cli.log('ran the command') }
    ...
    // edit old command
    let existingCommand = path.join('example-plugin', 'src', 'commands', 'cli', 'test.ts')
...
export default class extends Command {
  ...
  async run() { cli.log('ran the new cli:test') }
    ...
    // add topic
    let pjsonPath = path.join('example-plugin', 'package.json')
    ...
    await fs.outputJSON(path.join('example-plugin', 'package.json'), pjson, { spaces: 2 })
    jest.resetModules()
    // run new command
    ...
    // unlink plugin
    await run(['plugins:unlink', './example-plugin'])
