...
function buildHelp (c: ParsedCommand, config: Config): string {
  ...
  c.flags = convertFlagsFromV5(c.flags, config)
...
function convertFlagsFromV5 (flags: ?(LegacyFlag[] | {[name: string]: Flag}), config: Config): {[name: string]: any} {
  ...
  return converter.convertFlagsFromV5(flags)
}
  ...
  async convertToCached (): Promise<CachedPlugin> {
    ...
    const commands: CachedCommand[] = plugin.commands
      .map((c: ParsedCommand): CachedCommand => ({
        ...
        flags: convertFlagsFromV5(c.flags, this.config)
      }))
